...
import { Clock, CheckCircle, ChefHat, Bike, CheckCheck, AlertCircle, User, X, Check, UtensilsCrossed } from 'lucide-react';
...
const initialOrders: Record<OrderStatus, LiveOrder[]> = {
  pending: [
    { id: 'ord-p001', customer: 'Marco Ferretti', items: [{ name: 'Pizza Margherita', qty: 2 }, { name: 'Coca-Cola', qty: 2 }], total: 26.50, type: 'delivery', minutesAgo: 2, address: 'Via Roma 14, Milano' },
    { id: 'ord-p002', customer: 'Sara Conti', items: [{ name: 'Spaghetti Carbonara', qty: 1 }, { name: 'Tiramisù', qty: 1 }], total: 19.80, type: 'takeaway', minutesAgo: 4 },
    { id: 'ord-p003', customer: 'Luca Bianchi', items: [{ name: 'Tagliata di manzo', qty: 1 }, { name: 'Acqua nat.', qty: 2 }], total: 34.00, type: 'delivery', minutesAgo: 6, address: 'Corso Buenos Aires 55' },
  ],
  confirmed: [
    { id: 'ord-c001', customer: 'Giulia Marino', items: [{ name: 'Pizza Diavola', qty: 1 }, { name: 'Birra artigianale', qty: 1 }], total: 21.00, type: 'delivery', minutesAgo: 10, address: 'Via Torino 8' },
    { id: 'ord-c002', customer: 'Davide Ricci', items: [{ name: 'Antipasto misto', qty: 2 }], total: 18.50, type: 'takeaway', minutesAgo: 12 },
  ],
  preparing: [
    { id: 'ord-pr001', customer: 'Elena Galli', items: [{ name: 'Lasagne al forno', qty: 2 }, { name: 'Vino rosso', qty: 1 }], total: 42.00, type: 'delivery', minutesAgo: 18, address: 'Via Montenapoleone 3' },
    { id: 'ord-pr002', customer: 'Roberto Esposito', items: [{ name: 'Risotto ai funghi', qty: 1 }], total: 16.00, type: 'takeaway', minutesAgo: 22 },
    { id: 'ord-pr003', customer: 'Valentina Russo', items: [{ name: 'Pizza Quattro stagioni', qty: 2 }, { name: 'Fritto misto', qty: 1 }], total: 38.50, type: 'delivery', minutesAgo: 25, address: 'Viale Certosa 77' },
  ],
  delivering: [
    { id: 'ord-d001', customer: 'Antonio De Luca', items: [{ name: 'Penne all\'arrabbiata', qty: 1 }, { name: 'Dolce', qty: 1 }], total: 22.00, type: 'delivery', minutesAgo: 35, address: 'Via Padova 120' },
    { id: 'ord-d002', customer: 'Francesca Lombardi', items: [{ name: 'Branzino al forno', qty: 2 }], total: 44.00, type: 'delivery', minutesAgo: 38, address: 'Via Sarpi 9' },
  ],
};

const columns: { key: OrderStatus; label: string; icon: React.ReactNode; color: string }[] = [
  { key: 'pending', label: 'In Attesa', icon: <AlertCircle size={15} />, color: 'text-[var(--danger)]' },
  { key: 'confirmed', label: 'Confermati', icon: <CheckCircle size={15} />, color: 'text-[var(--info)]' },
  { key: 'preparing', label: 'In Preparazione', icon: <ChefHat size={15} />, color: 'text-[var(--warning)]' },
  { key: 'delivering', label: 'In Consegna', icon: <Bike size={15} />, color: 'text-[var(--success)]' },
];
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-semibold text-foreground truncate max-w-[110px]">{order.customer}</span>
                  </div>
                  <Badge variant={order.type === 'delivery' ? 'info' : 'neutral'} className="text-[10px]">
                    {order.type === 'delivery' ? '🛵 Consegna' : '🥡 Asporto'}
                  </Badge>
                </div>
                <ul className="space-y-0.5 mb-2.5">
                  {order.items.map((item, idx) => (
                    <li key={`${order.id}-item-${idx}`} className="text-xs text-muted-foreground flex justify-between">
                ...
                {order.address && (
                  <p className="text-[10px] text-muted-foreground mb-2 truncate">📍 {order.address}</p>
                )}
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold tabular-nums text-foreground">€ {order.total.toFixed(2)}</span>
                </div>
