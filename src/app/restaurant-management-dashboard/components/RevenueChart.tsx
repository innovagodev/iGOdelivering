    ...
    <div className="bg-card border border-border rounded-xl shadow-modal px-4 py-3 min-w-[140px]">
      ...
      {payload.map((entry, i) => (
        <div key={`tt-${i}`} className="flex items-center justify-between gap-4">
          <span className="text-xs text-muted-foreground">{entry.name === 'ricavi' ? 'Ricavi' : 'Ordini'}</span>
    ...
    <div className="bg-card rounded-xl border border-border shadow-card p-5">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={revenueData} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
          ...
          <XAxis dataKey="giorno" tick={{ fontSize: 12, fill: 'var(--muted-foreground)', fontFamily: 'var(--font-sans)' }} axisLine={false} tickLine={false} />
          <YAxis tick={{ fontSize: 11, fill: 'var(--muted-foreground)', fontFamily: 'var(--font-sans)' }} axisLine={false} tickLine={false} tickFormatter={(v) => `€${v}`} />
          <Tooltip content={<CustomTooltip />} />
          <Area type="monotone" dataKey="ricavi" stroke="var(--primary)" strokeWidth={2.5} fill="url(#gradRicavi)" dot={false} activeDot={{ r: 5, fill: 'var(--primary)', strokeWidth: 0 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
