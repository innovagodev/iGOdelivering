...
export default function MenuEditorTab({
  ...
  const handleOpenAddForm = (catName?: string) => {
    if (catName && catName !== 'Tutti') {
  ...
  const categoriesToRender = React.useMemo(() => {
    ...
    const itemCats = Array.from(new Set(filteredItems.map((i) => i.category)));
    const allCats = Array.from(new Set([...categories, ...itemCats]));
    return allCats;
  }, [activeCategory, categories, filteredItems]);

  const renderDishCard = (item: MenuItem) => {
    const globalIdx = filteredItems.findIndex((m) => m.id === item.id);
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between gap-2 mt-3 pt-3 border-t border-border/50">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() => handleEditDishClick(item.id)}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-muted text-muted-foreground hover:text-[var(--warning)] hover:bg-[var(--warning-bg)] rounded-xl text-sm font-semibold transition-all cursor-pointer"
          >
            <PauseCircle size={14} /> Sospendi Tutto
          </button>
          <button
            onClick={resumeAllDishes}
            className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white hover:bg-primary-hover rounded-xl text-sm font-semibold transition-all cursor-pointer"
          >
      ...
      {filteredItems.length === 0 ? (
          ...
          {filteredItems.map((item) => (
            <React.Fragment key={item.id}>
              {editingItemId === item.id ? (
                <div ref={itemFormRef} className="col-span-1 xl:col-span-2 bg-card border border-border rounded-2xl p-2 animate-fade-in">
                  <ItemForm
        ...
        <div className="space-y-6">
          {categoriesToRender.map((cat) => {
            ...
            const isCategoryHidden = hiddenCategories.has(cat);

            return (
              <div
                key={`cat-editor-block-${cat}`}
                className="space-y-3 bg-card border border-border/80 rounded-2xl p-4 shadow-card"
              >
                {/* Category Header */}
                <div className="flex items-center justify-between border-b border-border/60 pb-2.5">
                  ...
                  <button
                    type="button"
                    onClick={() => handleOpenAddForm(cat)}
                    className="text-xs font-bold text-primary hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    <Plus size={13} />
                    + Aggiungi Piatto in {cat}
                  </button>
                </div>

                {/* Grid of Dishes for this category */}
                {catItems.length === 0 ? (
      ...
      <div className="flex justify-center pt-2">
        <button
          ...
          className="bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-primary-hover transition-all shadow-sm cursor-pointer active:scale-95"
        >
          <Plus size={16} />
          + Aggiungi Piatto
        </button>
      ...
      {showAddItem && (
        <div ref={itemFormRef} className="bg-card border border-border rounded-2xl p-2 animate-fade-in">
          <ItemForm
            item={{
              ...emptyDraft(),
              category: activeCategory !== 'Tutti' ? activeCategory : (categories[0] || 'Pizza'),
            }}
            categories={categories}
            onSave={addMenuItem}
