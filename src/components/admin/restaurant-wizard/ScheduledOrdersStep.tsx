  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Toggle
          checked={scheduledOrders.enabled}
          onChange={() => updateScheduled('enabled', !scheduledOrders.enabled)}
          size="sm"
        />
      </div>
      ...
      {scheduledOrders.enabled && (
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <button
              onClick={() => toggleSection('altroExpanded')}
              className="w-full flex items-center justify-between px-5 py-4 hover:bg-muted/50 transition-colors"
            >
              <span className="font-semibold text-sm">Impostazioni Avanzate</span>
              {scheduledOrders.altroExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            </button>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      Nascondi &quot;Il prima possibile&quot;
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Obbliga il cliente a scegliere un orario specifico
                    </p>
                  </div>
                  <Toggle
                    checked={scheduledOrders.hideAsap}
                    onChange={() => updateScheduled('hideAsap', !scheduledOrders.hideAsap)}
                    size="sm"
                  />
                </div>
              </div>
